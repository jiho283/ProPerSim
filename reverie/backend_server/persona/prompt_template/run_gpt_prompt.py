...
def run_gpt_prompt_wake_up_hour(persona, test_input=None, verbose=False, client=None): 
  ...
  def __func_clean_up(gpt_response, prompt=""):
    for i in range(12, 0, -1):
      if f'{i}am' in gpt_response or f' {i}:00' in gpt_response:
        gpt_response = f'{i}am'
        break
      elif f'{i}pm' in gpt_response or f' {i}:00' in gpt_response:
        i = i + 12
        gpt_response = f'{i}am'
        break
  ...
  output = safe_generate_response_gpt4o(prompt, gpt_param, 5, fail_safe,
...
def run_gpt_prompt_generate_hourly_schedule(persona, 
  ...
  def create_prompt_input(persona, 
    ...
    intermission_str = f"Create a schedule for {persona.scratch.get_str_firstname()}'s entire day at home. All schedules are planned to take place at home, including the garden.\nConstruct a definite schedule for {curr_hour_str} without using speculative expressions like 'possibly' or 'or'.\nHere the originally intended hourly breakdown of"
  ...
  if 'AM] ' in output or 'PM] ' in output :
    if f'M] Activity: {persona.scratch.get_str_firstname()} ' in output:
      if f'M] Activity: {persona.scratch.get_str_firstname()} is ' in output:
        output = output.split(f'Activity: {persona.scratch.get_str_firstname()} is ')[1]
      else:
        output = output.split(f'Activity: {persona.scratch.get_str_firstname()} ')[1]
    else:
      if f'M] {persona.scratch.get_str_firstname()} is ' in output:
        output = output.split(f'M] {persona.scratch.get_str_firstname()} is ')[1]
      elif f'M] {persona.scratch.get_str_firstname()} ' in output:
        output = output.split(f'M] {persona.scratch.get_str_firstname()} ')[1]
...
def run_gpt_prompt_task_decomp(persona, 
  ...
  print ("?????")
  print (prompt)
  output = safe_generate_response_gpt4o(prompt, gpt_param, 5, get_fail_safe(),
                                   __func_validate, __func_clean_up, client=client)
...
# def run_gpt_prompt_make_evaluation(user_persona, agent_persona, retrieved=None, train_mode=None, client=None, first_suggestion=None):
#   def create_prompt_input(user_persona, agent_persona, retrieved, train_mode, first_suggestion=None): 
#     if "five_criteria" in train_mode:
#       if "revision" in train_mode:
#         prompt_template = "persona/prompt_template/v1/scoring_evaluation_w_mem_five_criteria_refine.txt"
#       elif 'ranking' in train_mode:
#         prompt_template = "persona/prompt_template/v1/scoring_evaluation_w_mem_five_criteria_wo_diversity_ranking.txt"
#       else:
#         prompt_template = "persona/prompt_template/v1/scoring_evaluation_w_mem_five_criteria_personalized_rubric.txt"
#     elif "ten_criteria" in train_mode:
#       prompt_template = "persona/prompt_template/v1/scoring_evaluation_w_mem_ten_criteria.txt"
#     elif "all_criteria" in train_mode:
#       prompt_template = "persona/prompt_template/v1/scoring_evaluation_w_mem_all_criteria_wo_diversity_ranking.txt"
#     elif "eight_criteria" in train_mode:
#       prompt_template = "persona/prompt_template/v1/scoring_evaluation_w_mem_eight_criteria_personalized_rubric.txt"
#     else:
#       prompt_template = "persona/prompt_template/v1/scoring_evaluation_w_mem.txt"
#     f = open(prompt_template, "r")
#     prompt = f.read()
#     f.close()
#     # import pdb;pdb.set_trace()
#     if agent_persona.scratch.user_action_for_suggestion == 'John is walking.':
#       user_action = f"{user_persona.scratch.curr_time.strftime('%A %B %d -- %I:%M:%S %p')}\n{user_persona.name} is making moves to do [{user_persona.scratch.act_description}]"
#     else:
#       if 'ing' in user_persona.scratch.act_description[:10]:
#         user_action = f"{user_persona.scratch.curr_time.strftime('%A %B %d -- %I:%M:%S %p')}\n{user_persona.name} is {user_persona.scratch.act_description}"
#       else:
#         user_action = f"{user_persona.scratch.curr_time.strftime('%A %B %d -- %I:%M:%S %p')}\n{user_persona.name} {user_persona.scratch.act_description}"

#     prompt = prompt.replace('<<USER_PERSONA>>', user_persona.scratch.get_str_iss())
#     prompt = prompt.replace('<<USER_ACTION>>', user_action)
#     if 'ranking' in train_mode:
#       prompt = prompt.replace('<<SUGGESTION_1>>', first_suggestion)
#       prompt = prompt.replace('<<SUGGESTION_2>>', agent_persona.scratch.suggestion)
#     else:
#       if first_suggestion != None:
#         prompt = prompt.replace('<<SUGGESTION>>', first_suggestion)
#       else:
#         prompt = prompt.replace('<<SUGGESTION>>', agent_persona.scratch.suggestion)
#     prompt = prompt.replace('<<AGENT_MEMORY>>', retrieved)

#     if 'personalized_rubric' in train_mode:
#       for key in user_persona.scratch.eval_criteria:
#         repl_key = key.replace('_', ' ')
#         prompt = prompt.replace(f'<<{repl_key}>>', user_persona.scratch.eval_criteria[key])
#     return prompt

#   def __func_clean_up(gpt_response, prompt=""):
#     after_score = gpt_response.split('"Score": ')[1]
#     score_reason = after_score.split(', "Reason": ')
#     score = score_reason[0]
#     reason = score_reason[1].split('}')[0]
#     return score, reason

#   def __func_validate(gpt_response, prompt=""): 
#     if len(gpt_response.strip()) < 10: 
#       return False
#     if '{"Score": ' not in gpt_response:
#       return False
...
#   def __func_clean_up_revision(gpt_response, prompt=""):
#     after_score = gpt_response.split('"Score": ')[1]
#     score_reason_revision = after_score.split(', "Reason": ')
#     score = score_reason_revision[0]
#     reason_revision = score_reason_revision[1].split(', "Improved Suggestion": ')
#     reason = reason_revision[0]
#     revision = reason_revision[1].split('}')[0]
#     return score, reason, revision

#   def __func_validate_revision(gpt_response, prompt=""): 
#     if len(gpt_response.strip()) < 20: 
#       return False
#     if '{"Score": ' not in gpt_response:
#       return False
#     if ', "Reason": ' not in gpt_response:
#       return False
#     if ', "Improved Suggestion": ' not in gpt_response:
#       return False
#     if "}" not in gpt_response:
#       return False
#     return True

#   def __func_clean_up_deducted_crit(gpt_response, prompt=""):
#     after_score = gpt_response.split('"Score": ')[1]
#     score_reason_revision = after_score.split(', "Reason": ')
#     score = score_reason_revision[0]
#     reason_revision = score_reason_revision[1].split(', "Deducted_Criteria": ')
#     reason = reason_revision[0]
#     revision = reason_revision[1].split('}')[0]
#     return score, reason, revision

#   def __func_validate_deducted_crit(gpt_response, prompt=""): 
#     if len(gpt_response.strip()) < 20: 
#       return False
#     if '{"Score": ' not in gpt_response:
#       return False
#     if ', "Reason": ' not in gpt_response:
#       return False
#     if ', "Deducted_Criteria": ' not in gpt_response:
#       return False
#     if "}" not in gpt_response:
#       return False
#     return True
...
#   def __func_clean_up_ranking(gpt_response, prompt=""):
#     after_score = gpt_response.split('"Score #1": ')[1]
#     score_reason_12 = after_score.split(', "Reason #1": ')
#     score_1 = score_reason_12[0]
#     score_reason_1 = score_reason_12[1].split(', "Score #2": ')
#     reason_1 = score_reason_1[0]
#     score_reason_2 = score_reason_1[1].split(', "Reason #2": ')
#     score_2 = score_reason_2[0]
#     reason_2 = score_reason_2[1].split('}')[0]
#     return (score_1, reason_1), (score_2, reason_2), ""

#   def __func_validate_ranking(gpt_response, prompt=""): 
#     if len(gpt_response.strip()) < 30: 
#       return False
#     if '{"Score #1": ' not in gpt_response:
#       return False
#     if ', "Reason #1": ' not in gpt_response:
#       return False
#     if ', "Score #2": ' not in gpt_response:
#       return False
#     if ', "Reason #2": ' not in gpt_response:
#       return False
#     if "}" not in gpt_response:
#       return False
#     return True
...
#   def get_fail_safe(): 
#     fs = 3, "Evaluation Failed"
#     return fs
  
#   def get_fail_safe_revision(): 
#     fs = 3, "Evaluation Failed", "N/A"
#     return fs

#   def get_fail_safe_deducted_crit(): 
#     fs = 3, "Evaluation Failed", "N/A"
#     return fs
  
#   def get_fail_safe_deducted_ranking(): 
#     fs = (3, "Evaluation Failed"), (3, "Evaluation Failed"), "[]"
#     return fs

#   gpt_param = {"engine": "text-davinci-003", "max_tokens": 15, 
#                "temperature": 0, "top_p": 1, "stream": False,
#                "frequency_penalty": 0, "presence_penalty": 0, "stop": None}
#   prompt = create_prompt_input(user_persona, agent_persona, retrieved, train_mode, first_suggestion)

#   revision = ""
#   if "revision" in train_mode:
#     fail_safe = get_fail_safe_revision()
#     score, reason, revision = safe_generate_response_gpt4o(prompt, gpt_param, 5, fail_safe,
#                                     __func_validate_revision, __func_clean_up_revision, client=client)
#     print('Retrieved:', prompt)
#     print ("Score:", score)
#     print("Reason:", reason)
#     print("Revision:", revision)
#   elif "ranking" in train_mode:
#     fail_safe = get_fail_safe_deducted_ranking()
#     score, reason, revision = safe_generate_response_gpt4o(prompt, gpt_param, 5, fail_safe,
#                                     __func_validate_ranking, __func_clean_up_ranking, client=client)
#     print('Retrieved:', prompt)
#     print ("Score #1/Reason #1:", score)
#     print("Score #2/Reason #2:", reason)
#     # print("Better:", revision)
#   else:
#     fail_safe = get_fail_safe_deducted_crit()
#     if '_gemini-2.0' in train_mode:
#       score, reason, revision = safe_generate_response_gemini20(prompt, gpt_param, 5, fail_safe,
#                                     __func_validate_deducted_crit, __func_clean_up_deducted_crit, google_client=user_persona.gemini)
#     elif '_gemini-2.5' in train_mode:
#       score, reason = safe_generate_response_gemini25(prompt, gpt_param, 5, fail_safe,
#                                     __func_clean_up_deducted_crit, __func_clean_up_deducted_crit, google_client=user_persona.gemini)
#     elif '_gpt-4o-mini':
#       score, reason, revision = safe_generate_response(prompt, gpt_param, 5, fail_safe,
#                                     __func_validate_deducted_crit, __func_clean_up_deducted_crit, client=client)
#     elif '_o3-mini':
#       score, reason, revision = safe_generate_response_o3mini(prompt, gpt_param, 5, fail_safe,
#                                     __func_validate_deducted_crit, __func_clean_up_deducted_crit, client=client)
#     elif '_o4-mini':
#       score, reason, revision = safe_generate_response_o4mini(prompt, gpt_param, 5, fail_safe,
#                                     __func_validate_deducted_crit, __func_clean_up_deducted_crit, client=client)
#     else:
#       score, reason, revision = safe_generate_response_gpt4o(prompt, gpt_param, 5, fail_safe,
#                                     __func_validate_deducted_crit, __func_clean_up_deducted_crit, client=client)
...
#     print ("Score:", score)
#     print("Reason:", reason)
#     print("Deducted Reason:", revision)

#   return score, reason, revision


def run_gpt_prompt_make_evaluation_seperate(user_persona, agent_persona, retrieved=None, train_mode=None, client=None, first_suggestion=None):
  def create_prompt_input(user_persona, agent_persona, retrieved, train_mode, category, first_suggestion=None): 
    if category == "Personal_Preference":
      prompt_template = "persona/prompt_template/v1/scoring_evaluation_w_mem_five_criteria_personalized_rubric_preference.txt"
    elif category == "Frequency":
      prompt_template = "persona/prompt_template/v1/scoring_evaluation_w_mem_five_criteria_personalized_rubric_frequency.txt"
    elif category == "Timing":
      prompt_template = "persona/prompt_template/v1/scoring_evaluation_w_mem_five_criteria_personalized_rubric_timing.txt"
    elif category == "Communication_&_Safety":
      prompt_template = "persona/prompt_template/v1/scoring_evaluation_w_mem_five_criteria_personalized_rubric_communication.txt"
    elif category == "Frequency_least":
      prompt_template = "persona/prompt_template/v1/scoring_evaluation_w_mem_five_criteria_personalized_rubric_frequency_least.txt"
    ...
    prompt = prompt.replace(f'<<{repl_key}>>', user_persona.scratch.eval_criteria[category])
    return prompt
  ...
  for category in user_persona.scratch.eval_criteria:
    prompt = create_prompt_input(user_persona, agent_persona, retrieved, train_mode, category, first_suggestion)
    fail_safe = get_fail_safe()
    if '_gemini-2.0' in train_mode:
      score, reason = safe_generate_response_gemini20(prompt, gpt_param, 5, fail_safe,
                                    __func_validate, __func_clean_up, google_client=user_persona.gemini)
    elif '_gemini-2.5' in train_mode:
      score, reason = safe_generate_response_gemini25(prompt, gpt_param, 5, fail_safe,
                                    __func_validate, __func_clean_up, google_client=user_persona.gemini)
    elif '_gpt-4o-mini':
      score, reason = safe_generate_response(prompt, gpt_param, 5, fail_safe,
                                    __func_validate, __func_clean_up, client=client)
    elif '_o3-mini':
      score, reason = safe_generate_response_o3mini(prompt, gpt_param, 5, fail_safe,
                                    __func_validate, __func_clean_up, client=client)
    elif '_o4-mini':
      score, reason = safe_generate_response_o4mini(prompt, gpt_param, 5, fail_safe,
                                    __func_validate, __func_clean_up, client=client)
    else:
      score, reason = safe_generate_response_gpt4o(prompt, gpt_param, 5, fail_safe,
                                    __func_validate, __func_clean_up, client=client)
    
    if '"' == reason[0] and '"' == reason[-1] and len(reason) > 3:
      reason = reason[1:-1]

    if category == 'Frequency':
      for_freq = [score, reason]
      continue
    if category == "Frequency_least":
      if float(for_freq[0]) == float(1) and float(score) == float(1):
        score = for_freq[0]
        reason = for_freq[1]
      elif float(for_freq[0]) == float(0):
        score = for_freq[0]
        reason = for_freq[1]

    total_reason += " " + reason
    total_score += float(score)
    total_deducted[category.replace('_least', '')] = {'score': score, 'reason': reason}
    print('Retrieved:', prompt)
  total_score = round(total_score, 1)
  
  print ("Score:", total_score)
  print("Reason:", total_reason)

  return str(total_score), total_reason, total_deducted


def run_gpt_prompt_make_revision(user_persona, agent_persona, score, reason, retrieved=None, train_mode=None, client=None):
  def create_prompt_input(user_persona, agent_persona, score, reason, retrieved, train_mode): 
    prompt_template = "persona/prompt_template/v1/user_suggestion_revision.txt"
    f = open(prompt_template, "r")
    prompt = f.read()
    f.close()
    # import pdb;pdb.set_trace()
    ...
    prompt = prompt.replace('<<PREFERENCE_RUBRIC>>', preference_rubric)

    return prompt

  def __func_clean_up(gpt_response, prompt=""):
    revision = gpt_response.split('"Revision": ')[1]
    reason = revision.split('}')[0]
    return reason

  def __func_validate(gpt_response, prompt=""): 
    if len(gpt_response.strip()) < 10: 
      return False
    if '{"Revision": ' not in gpt_response:
      return False
    if "}" not in gpt_response:
      return False
    return True
  ...
  prompt = create_prompt_input(user_persona, agent_persona, score, reason, retrieved, train_mode)
  fail_safe = get_fail_safe()
  if '_gemini-2.0' in train_mode:
    revision = safe_generate_response_gemini20(prompt, gpt_param, 5, fail_safe,
                                  __func_validate, __func_clean_up, google_client=user_persona.gemini)
  elif '_gemini-2.5' in train_mode:
    revision = safe_generate_response_gemini25(prompt, gpt_param, 5, fail_safe,
                                  __func_validate, __func_clean_up, google_client=user_persona.gemini)
  elif '_gpt-4o-mini':
    revision = safe_generate_response(prompt, gpt_param, 5, fail_safe,
                                  __func_validate, __func_clean_up, client=client)
  elif '_o3-mini':
    revision = safe_generate_response_o3mini(prompt, gpt_param, 5, fail_safe,
                                  __func_validate, __func_clean_up, client=client)
  elif '_o4-mini':
    revision = safe_generate_response_o4mini(prompt, gpt_param, 5, fail_safe,
                                  __func_validate, __func_clean_up, client=client)
  else:
    revision = safe_generate_response_gpt4o(prompt, gpt_param, 5, fail_safe,
                                  __func_validate, __func_clean_up, client=client)
  
  if '"' == revision[0] and '"' == revision[-1] and len(revision) > 3:
...
def run_gpt_prompt_hourly_sum(time_sugg_list, train_mode, client=None):
  def __func_clean_up(gpt_response, prompt=""):
  ...
  prompt_template = "persona/prompt_template/v1/summarization_hourly.txt" 
  f = open(prompt_template, "r")
  prompt = f.read()
  f.close()
...
def run_gpt_prompt_4hourly_sum(time_sugg_list, train_mode, client=None):
  def __func_clean_up(gpt_response, prompt=""):
    after_summ = gpt_response.split('"Summary": ')[1]
    summ = after_summ.split('}')[0]
    return summ
  ...
  prompt_template = "persona/prompt_template/v1/summarization_4hourly.txt" 
  f = open(prompt_template, "r")
  prompt = f.read()
  f.close()

  histories = ""
  for d, t in time_sugg_list:
    histories += t
...
def run_gpt_prompt_hourly_sum_agent(time_sugg_list, train_mode, client=None):
  def __func_clean_up(gpt_response, prompt=""):
    after_summ = gpt_response.split('"Summary": ')[1]
    summ = after_summ.split('}')[0]
    return summ
  ...
  if '_AS' in train_mode and 'C' in train_mode:
    prompt_template = "persona/prompt_template/v1/agent_summ_hourly_score.txt" 
  else:
    prompt_template = "persona/prompt_template/v1/agent_summ_hourly.txt" 
  f = open(prompt_template, "r")
  prompt = f.read()
  f.close()
...
def run_gpt_prompt_4hourly_sum_agent(time_sugg_list, train_mode, client=None):
  def __func_clean_up(gpt_response, prompt=""):
    after_summ = gpt_response.split('"Summary": ')[1]
    summ = after_summ.split('}')[0]
    return summ
  ...
  if '_AS' in train_mode and 'C' in train_mode:
    prompt_template = "persona/prompt_template/v1/agent_summ_4hourly_score.txt" 
  else:
    prompt_template = "persona/prompt_template/v1/agent_summ_4hourly.txt" 
  f = open(prompt_template, "r")
  prompt = f.read()
  f.close()
