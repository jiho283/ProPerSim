...
def extract_relevance(persona, nodes, focal_pt, client=None, is_eval_suggest=False): 
  """
  ...
  """
  if is_eval_suggest:
    target_persona_memory = persona.e_mem
  else:
    target_persona_memory = persona.a_mem
  focal_embedding = get_embedding(focal_pt, client=client)

  relevance_out = dict()
  for count, node in enumerate(nodes): 
    node_embedding = target_persona_memory.embeddings[node.embedding_key]
    relevance_out[node.node_id] = cos_sim(node_embedding, focal_embedding)

  return relevance_out


def new_retrieve(persona, focal_points, n_count=5, client=None, is_eval_suggest=False): 
  """
  ...
  for focal_pt in focal_points: 
    ...
    if is_eval_suggest:
      target_persona_memory = persona.e_mem
      if len(target_persona_memory.seq_event) > 4:
        recent_seq_events = target_persona_memory.seq_event[:4]
        retrieved['recent_events'] = recent_seq_events
      else:
        recent_seq_events = target_persona_memory.seq_event[:len(target_persona_memory.seq_event)]
        retrieved['recent_events'] = recent_seq_events
    else:
      target_persona_memory = persona.a_mem

    nodes = [[i.last_accessed, i]
            for i in target_persona_memory.seq_event + target_persona_memory.seq_thought
    ...
    relevance_out = extract_relevance(persona, nodes, focal_pt, client=client, is_eval_suggest=is_eval_suggest)
    ...
    for key, val in master_out.items(): 
      print (target_persona_memory.id_to_node[key].embedding_key, val)
    ...
    # For retrieval, should use it
    # master_out = top_highest_x_values(master_out, n_count)
    # master_nodes = [target_persona_memory.id_to_node[key] 
  ...
  if is_eval_suggest:
    if len(target_persona_memory.seq_event) > 4:
      retrieved['recent_events'] = target_persona_memory.seq_event[:4]
    else:
      retrieved['recent_events'] = target_persona_memory.seq_event[:len(target_persona_memory.seq_event)]
