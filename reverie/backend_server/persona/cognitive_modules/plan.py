...
def generate_wake_up_hour(persona, client=None):
  """
  ...
  return int(run_gpt_prompt_wake_up_hour(persona, client=client)[0])


def generate_first_daily_plan(persona, wake_up_hour, client=None): 
  """
  ...
  return run_gpt_prompt_daily_plan(persona, wake_up_hour, client=client)[0]


def generate_hourly_schedule(persona, wake_up_hour, client=None): 
  """
      ...
      for count, curr_hour_str in enumerate(hour_str): 
        if wake_up_hour > 0: 
          ...
          n_m1_activity += [run_gpt_prompt_generate_hourly_schedule(
                          persona, curr_hour_str, n_m1_activity, hour_str, client=client)[0]]
...
def generate_task_decomp(persona, task, duration, client=None): 
  """
  ...
  return run_gpt_prompt_task_decomp(persona, task, duration, client=client)[0]


def generate_action_sector(act_desp, persona, maze, client=None): 
  """TODO 
  ...
  return run_gpt_prompt_action_sector(act_desp, persona, maze, client=client)[0]


def generate_action_arena(act_desp, persona, maze, act_world, act_sector, client=None): 
  """TODO 
  ...
  return run_gpt_prompt_action_arena(act_desp, persona, maze, act_world, act_sector, client=client)[0]


def generate_action_game_object(act_desp, act_address, persona, maze, client=None):
  """TODO
  ...
  if not persona.s_mem.get_str_accessible_arena_game_objects(act_address): 
    return "<random>"
  return run_gpt_prompt_action_game_object(act_desp, persona, maze, act_address, client=client)[0]


def generate_action_pronunciatio(act_desp, persona, client=None): 
  """TODO 
  ...
  return "🙂"
  # try: 
  #   x = run_gpt_prompt_pronunciatio(act_desp, persona, client=client)[0]
  # except: 
  #   x = "🙂"
...
def generate_action_event_triple(act_desp, persona, client=None): 
  """TODO 
  ...
  return run_gpt_prompt_event_triple(act_desp, persona, client=client)[0]


def generate_act_obj_desc(act_game_object, act_desp, persona, client=None): 
  if debug: print ("GNS FUNCTION: <generate_act_obj_desc>")
  return run_gpt_prompt_act_obj_desc(act_game_object, act_desp, persona, client=client)[0]


def generate_act_obj_event_triple(act_game_object, act_obj_desc, persona, client=None): 
  if debug: print ("GNS FUNCTION: <generate_act_obj_event_triple>")
  return run_gpt_prompt_act_obj_event_triple(act_game_object, act_obj_desc, persona, client=client)[0]


def generate_convo(maze, init_persona, target_persona, client=None): 
  curr_loc = maze.access_tile(init_persona.scratch.curr_tile)

  # convo = run_gpt_prompt_create_conversation(init_persona, target_persona, curr_loc)[0]
  # convo = agent_chat_v1(maze, init_persona, target_persona)
  convo = agent_chat_v2(maze, init_persona, target_persona, client=client)
  all_utt = ""
...
def generate_convo_summary(persona, convo, client=None): 
  convo_summary = run_gpt_prompt_summarize_conversation(persona, convo, client=client)[0]
  return convo_summary


def generate_decide_to_talk(init_persona, target_persona, retrieved, client=None): 
  x =run_gpt_prompt_decide_to_talk(init_persona, target_persona, retrieved, client=client)[0]
...
def generate_decide_to_react(init_persona, target_persona, retrieved, client=None): 
  if debug: print ("GNS FUNCTION: <generate_decide_to_react>")
  return run_gpt_prompt_decide_to_react(init_persona, target_persona, retrieved, client=client)[0]


def generate_new_decomp_schedule(persona, inserted_act, inserted_act_dur,  start_hour, end_hour, client=None): 
  ...
  return run_gpt_prompt_new_decomp_schedule(persona, 
                                            ...
                                            end_time_hour,
                                            inserted_act,
                                            inserted_act_dur, client=client)[0]
...
def revise_identity(persona, client=None): 
  p_name = persona.scratch.name

  focal_points = [f"{p_name}'s plan for {persona.scratch.get_str_curr_date_str()}.",
                  f"Important recent events for {p_name}'s life."]
  retrieved = new_retrieve(persona, focal_points, client=client)
  ...
  plan_note = ChatGPT_single_request(plan_prompt, client=client)
  # print (plan_note)

  thought_prompt = statements + "\n"
  thought_prompt += f"Given the statements above, how might we summarize {p_name}'s feelings about their days up to now?\n\n"
  thought_prompt += f"Write the response from {p_name}'s perspective."
  thought_note = ChatGPT_single_request(thought_prompt, client=client)
  # print (thought_note)

  currently_prompt = f"{p_name}'s status from {(persona.scratch.curr_time - datetime.timedelta(days=1)).strftime('%A %B %d')}:\n"
  currently_prompt += f"{persona.scratch.current_interests}\n\n"
  ...
  new_currently = ChatGPT_single_request(currently_prompt, client=client)

  persona.scratch.current_interests = new_currently
  ...
  new_daily_req = ChatGPT_single_request(daily_req_prompt, client=client)
  new_daily_req = new_daily_req.replace('\n', ' ')
  print ("WE ARE HERE!!!", new_daily_req)
  persona.scratch.daily_plan_req = new_daily_req


def _long_term_planning(persona, new_day, client=None): 
  """
  ...
  wake_up_hour = generate_wake_up_hour(persona, client=client)
  ...
  if new_day == "First day": 
    ...
    persona.scratch.daily_req = generate_first_daily_plan(persona, 
                                                          wake_up_hour, client=client)
  elif new_day == "New day":
    revise_identity(persona, client=client)
  ...
  keywords = set(["plan"])
  thought_poignancy = 5
  thought_embedding_pair = (thought, get_embedding(thought, client=client))
...
def _determine_action(persona, maze, client=None): 
  """
    ...
    if act_dura >= 60: 
      ...
      if determine_decomp(act_desp, act_dura): 
        persona.scratch.f_daily_schedule[curr_index:curr_index+1] = (
                            generate_task_decomp(persona, act_desp, act_dura, client=client))
  ...
  act_sector = generate_action_sector(act_desp, persona, maze, client=client)
  act_arena = generate_action_arena(act_desp, persona, maze, act_world, act_sector, client=client)
  act_address = f"{act_world}:{act_sector}:{act_arena}"
  act_game_object = generate_action_game_object(act_desp, act_address,
                                                persona, maze, client=client)
  new_address = f"{act_world}:{act_sector}:{act_arena}:{act_game_object}"
  act_pron = generate_action_pronunciatio(act_desp, persona, client=client)
  act_event = generate_action_event_triple(act_desp, persona, client=client)
  # Persona's actions also influence the object states. We set those up here. 
  act_obj_desp = generate_act_obj_desc(act_game_object, act_desp, persona, client=client)
  act_obj_pron = generate_action_pronunciatio(act_obj_desp, persona, client=client)
  act_obj_event = generate_act_obj_event_triple(act_game_object, 
                                                act_obj_desp, persona, client=client)
...
def _should_react(persona, retrieved, personas, client=None): 
  """
  ...
  """
  def lets_talk(init_persona, target_persona, retrieved, client=None):
    ...
    if generate_decide_to_talk(init_persona, target_persona, retrieved, client=client): 

      return True

    return False

  def lets_react(init_persona, target_persona, retrieved, client=None): 
    if (not target_persona.scratch.act_address 
        or not target_persona.scratch.act_description
        or not init_persona.scratch.act_address
        or not init_persona.scratch.act_description): 
    ...
    react_mode = generate_decide_to_react(init_persona, 
                                          target_persona, retrieved, client=client)
  ...
  if ":" not in curr_event.subject: 
    ...
    if lets_talk(persona, personas[curr_event.subject], retrieved, client=client):
      return f"chat with {curr_event.subject}"
    react_mode = lets_react(persona, personas[curr_event.subject], 
                            retrieved, client=client)
    return react_mode
  return False
...
def _create_react(persona, inserted_act, inserted_act_dur,
                  ...
                  act_obj_event, act_start_time=None, client=None): 
  p = persona 
  ...
  ret = generate_new_decomp_schedule(p, inserted_act, inserted_act_dur, 
                                       start_hour, end_hour, client=client)
...
def _chat_react(maze, persona, focused_event, reaction_mode, personas, client=None):
  ...
  convo, duration_min = generate_convo(maze, init_persona, target_persona, client=client)
  convo_summary = generate_convo_summary(init_persona, convo, client=client)
  ...
  for role, p in [("init", init_persona), ("target", target_persona)]: 
    ...
    _create_react(p, inserted_act, inserted_act_dur,
      ...
      act_pronunciatio, act_obj_description, act_obj_pronunciatio, 
      act_obj_event, act_start_time, client=client)


def _wait_react(persona, reaction_mode, client=None): 
  p = persona
  ...
  _create_react(p, inserted_act, inserted_act_dur,
    ...
    act_pronunciatio, act_obj_description, act_obj_pronunciatio, act_obj_event, client=client)


def plan(persona, maze, personas, new_day, retrieved, client=None): 
  """
  ...
  if new_day: 
    _long_term_planning(persona, new_day, client=client)

  # PART 2: If the current action has expired, we want to create a new plan.
  if persona.scratch.act_check_finished(): 
    _determine_action(persona, maze, client=client)
  ...
  #         c) False
  if focused_event: 
    reaction_mode = _should_react(persona, focused_event, personas, client=client)
    if reaction_mode: 
      # If we do want to chat, then we generate conversation 
      if reaction_mode[:9] == "chat with":
        _chat_react(maze, persona, focused_event, reaction_mode, personas, client=client)
      elif reaction_mode[:4] == "wait": 
        _wait_react(persona, reaction_mode, client=client)
