...
def generate_focal_points(persona, n=3, client=None): 
  ...
  return run_gpt_prompt_focal_pt(persona, statements, n, client=client)[0]


def generate_insights_and_evidence(persona, nodes, n=5, client=None): 
  ...
  ret = run_gpt_prompt_insight_and_guidance(persona, statements, n, client=client)[0]
...
def generate_planning_thought_on_convo(persona, all_utt, client=None):
  if debug: print ("GNS FUNCTION: <generate_planning_thought_on_convo>")
  return run_gpt_prompt_planning_thought_on_convo(persona, all_utt, client=client)[0]


def generate_memo_on_convo(persona, all_utt, client=None):
  if debug: print ("GNS FUNCTION: <generate_memo_on_convo>")
  return run_gpt_prompt_memo_on_convo(persona, all_utt, client=client)[0]




def run_reflect(persona, client=None):
  """
  ...
  focal_points = generate_focal_points(persona, 3, client=client)
  ...
  for focal_pt, nodes in retrieved.items(): 
    ...
    thoughts = generate_insights_and_evidence(persona, nodes, 5, client=client)
    for thought, evidence in thoughts.items(): 
      created = persona.scratch.curr_time
      expiration = persona.scratch.curr_time + datetime.timedelta(days=30)
      s, p, o = generate_action_event_triple(thought, persona, client=client)
      keywords = set([s, p, o])
      thought_poignancy = generate_poig_score(persona, "thought", thought, client=client)
...
def reflect(persona, client=None):
  """
  ...
  if reflection_trigger(persona): 
    run_reflect(persona, client=client)
  ...
  if persona.scratch.chatting_end_time: 
    ...
    if persona.scratch.curr_time + datetime.timedelta(0,10) == persona.scratch.chatting_end_time: 
      ...
      planning_thought = generate_planning_thought_on_convo(persona, all_utt, client=client)
      planning_thought = f"For {persona.scratch.name}'s planning: {planning_thought}"

      created = persona.scratch.curr_time
      expiration = persona.scratch.curr_time + datetime.timedelta(days=30)
      s, p, o = generate_action_event_triple(planning_thought, persona, client=client)
      keywords = set([s, p, o])
      thought_poignancy = generate_poig_score(persona, "thought", planning_thought, client=client)
      thought_embedding_pair = (planning_thought, get_embedding(planning_thought, client=client))
      ...
      memo_thought = generate_memo_on_convo(persona, all_utt, client=client)
      memo_thought = f"{persona.scratch.name} {memo_thought}"

      created = persona.scratch.curr_time
      expiration = persona.scratch.curr_time + datetime.timedelta(days=30)
      s, p, o = generate_action_event_triple(memo_thought, persona, client=client)
      keywords = set([s, p, o])
      thought_poignancy = generate_poig_score(persona, "thought", memo_thought, client=client)
      thought_embedding_pair = (memo_thought, get_embedding(memo_thought, client=client))
