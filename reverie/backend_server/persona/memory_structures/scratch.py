...
class Scratch: 
  def __init__(self, f_saved): 
    ...
    self.big_five_personality_traits = None
    # self.innate = None
    # L1 stable traits.
    self.background = None
    # L2 external implementation. 
    self.long_term_goals = None
    self.current_interests = None
    self.lifestyle = None
    ...
    self.user_action_for_suggestion = None
    self.suggestion_eval_result = None
    self.eval_criteria = {}
    ...
    if check_if_file_exists(f_saved): 
      ...
      self.big_five_personality_traits = scratch_load["big_five_personality_traits"]
      self.background = scratch_load["background"]
      self.long_term_goals = scratch_load["long_term_goals"]
      self.current_interests = scratch_load["current_interests"]
      ...
      self.suggestion = scratch_load["suggestion"]
      self.user_action_for_suggestion = scratch_load["user_action_for_suggestion"]
      self.suggestion_eval_result = scratch_load["suggestion_eval_result"]

      if "criteria" in scratch_load:
        self.eval_criteria = scratch_load["criteria"]
        self.eval_criteria['Frequency_least'] = self.eval_criteria['Frequency']
  ...
  def save(self, out_json):
    ...
    scratch["big_five_personality_traits"] = self.big_five_personality_traits
    scratch["background"] = self.background
    scratch["long_term_goals"] = self.long_term_goals
    # scratch["innate"] = self.innate
    scratch["current_interests"] = self.current_interests
  ...
  def get_str_iss(self): 
    ...
    EXAMPLE STR OUTPUT
      "Name: Dolores Heitmiller
       Age: 28
       big_five_personality_traits traits: hard-edged, independent, loyal
       background traits: Dolores is a painter who wants live quietly and paint 
         while enjoying her everyday life.
       current_interests: Dolores is preparing for her first solo show. She mostly 
         works from home.
    ...
    commonset += f"Big Five Personality Traits: {self.big_five_personality_traits}\n"
    commonset += f"Background: {self.background}\n"
    # commonset += f"Innate: {self.innate}\n"
    commonset += f"Current interests: {self.current_interests}\n"
    commonset += f"Lifestyle: {self.lifestyle}\n"
    commonset += f"Long-term goals: {self.long_term_goals}\n"
  ...
  def get_str_big_five_personality_traits(self): 
    return self.big_five_personality_traits


  def get_str_background(self): 
    return self.background
  
  def get_str_long_term_goals(self): 
    return self.long_term_goals 


  def get_str_current_interests(self): 
