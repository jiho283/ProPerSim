...
from persona.memory_structures.eval_memory import *
...
from persona.cognitive_modules.evaluation import *

class Persona: 
  def __init__(self, name, folder_mem_saved=False, client=None, gemini=None):
    # PERSONA BASE STATE 
    ...
    self.events_hourly = []
    self.memory_hourly = []
    self.memory_4hourly = deque(maxlen=3)
  ...
  def plan(self, maze, personas, new_day, retrieved):
    ...
    """
    return plan(self, maze, personas, new_day, retrieved, self.client)
  ...
  def eval_suggestion(self, personas, train_mode, first_suggestion=None):
    return eval_suggestion(self, personas, self.client, train_mode, first_suggestion)
  
  def suggestion_revision(self, agent_persona, score, reason, train_mode):
    return revise_suggestion(self, agent_persona, score, reason, train_mode, self.client)
  
  def perceive_suggestion_eval(self, personas, action, suggestion, score, score_reason, revision, train_mode):
    return perceive_suggestion_eval(personas, action, suggestion, score, score_reason, revision, train_mode, self.client)
  
  def eval_suggestion_wo_record(self, personas, train_mode):
    return eval_suggestion_wo_record(self, personas, self.client, train_mode)

  def open_convo_session(self, convo_mode): 
    open_convo_session(self, convo_mode, self.client)
  
  def init_e_mem(self, folder_mem_saved):
    self.recent_events_hourly = deque(maxlen=24)
    self.summary_recent_events_hourly = ""
    self.events_hourly = []
    self.memory_hourly = []
