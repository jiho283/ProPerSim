...
def _create_react(persona, inserted_act, inserted_act_dur,
  ...
  p = persona 
  p.scratch.f_daily_schedule[0:1] = ""
...
def _chat_react(maze, persona, focused_event, personas, client=None):
  ...
  for role, p in [("init", init_persona)]: 
    if role == "init": 
      act_address = f"<persona> John Lin"
      act_event = (p.name, "chat with", "John Lin")
      chatting_with = "John Lin"
      chatting_with_buffer = {}
      chatting_with_buffer["John Lin"] = 800
...
def plan(persona, maze, personas, new_day, retrieved, client=None, user_action=None, train_mode=None): 
  """
  ...
  _chat_react(maze, persona, focused_event, personas, client=client)
