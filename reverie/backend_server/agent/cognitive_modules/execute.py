...
def execute(persona, maze, personas, plan): 
  ...
  if not persona.scratch.act_path_set: 
    ...
    for persona_name, persona_body in personas.items():
      if persona_name == 'John Lin':
        if 'sleeping' == persona_body.scratch.act_description:
    ...
    persona.scratch.act_description = 'suggestion'
    ...
    if "<persona>" in plan: 
      ...
      target_tiles = [(target_p_tile[0], target_p_tile[1] + 1)]
    ...
    #   target_tiles = random.sample(list(target_tiles), 1)
      ...
      if plan not in maze.address_tiles: 
        maze.address_tiles["Lin family's house:common room:common room table"] #ERRORRRRRRR
      else: 
        target_tiles = maze.address_tiles[plan]
    ...
    print('here!!!: ', persona.scratch.planned_path)
    print('target tiles:: ', target_tiles)
  ...
  description += f" @ {persona.scratch.act_address}"
  execution = ret, persona.scratch.act_pronunciatio, description
  # print('result: ', execution, persona.scratch.planned_path)
  return execution
