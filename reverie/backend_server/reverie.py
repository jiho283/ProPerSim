...
class ReverieServer: 
  def __init__(self, 
    ...
    if GOOGLE_API_KEY != "":
      ...
      if 'gemini' in self.train_mode:
        self.gemini = google_client

    # self.train_mode = "gpt-4o-mini_plantest"
    # self.train_mode = "gpt-4o-mini_evaltest_reasonright_ret_suggest"
  ...
  def start_server(self, int_counter): 
    ...
    action, suggestion = None, None
    eval_res_score, eval_res_reason = None, None
    john_tile = 0
    is_john_walking = 0
    logging_tokenizer = self.load_logging_tokenizer()

    while int_counter > 0: 
      if '_novis' in self.train_mode:
        self.generate_fake_environment()
      ...
      # For only-backend version (use this env_retrieved)
        if env_retrieved: 
          # This is where we go through <game_obj_cleanup> to clean up all 
          # object actions that were used in this cylce. 
      # For only-backend version (try)
          ...
          if "00:02:30" in self.curr_time.strftime("%B %d, %Y, %H:%M:%S"):
            self.personas['Eddy Lin'].update_suggestion_model(self.train_mode)
            persona_folder = f"{sim_folder}/personas/John Lin"
            self.personas['John Lin'].init_e_mem(persona_folder)
          ...
          for persona_name, persona in self.personas.items(): 
            ...
            if "Eddy Lin" == persona_name:
              if 'ranking' in self.train_mode:
                if 'recrev' in self.train_mode:
                  action, first_suggestion = persona.suggest(self.personas, self.train_mode, is_john_walking=is_john_walking, movement_path=f"{sim_folder}/movement/", 
                                                    step=self.step, curr_time=self.curr_time.strftime("%A %B %d -- %I:%M:%S %p"))

                  eval_res_score, eval_res_reason, deducted_reason = self.personas['John Lin'].eval_suggestion_wo_record(self.personas, train_mode=self.train_mode.replace('ranking', ''))
                  if eval_res_score and first_suggestion:
                    if float(eval_res_score) == 4.0:
                      persona.scratch.suggestion = None
                      action, suggestion = persona.suggest(self.personas, self.train_mode, is_john_walking=is_john_walking, movement_path=f"{sim_folder}/movement/", 
                                                      step=self.step, curr_time=self.curr_time.strftime("%A %B %d -- %I:%M:%S %p"))
                      eval_res_score_2, eval_res_reason_2, deducted_reason_2 = self.personas['John Lin'].eval_suggestion(self.personas, train_mode=self.train_mode.replace('ranking', ''))
                    
                    else:
                      suggestion = self.personas['John Lin'].suggestion_revision(persona, eval_res_score, eval_res_reason, self.train_mode)
                      persona.scratch.suggestion = suggestion
                      eval_res_score_2, eval_res_reason_2, deducted_reason_2 = self.personas['John Lin'].eval_suggestion(self.personas, train_mode=self.train_mode.replace('ranking', 'secondsugg'))
                      persona.scratch.suggestion = first_suggestion
                      self.personas['John Lin'].scratch.suggestion = first_suggestion

                      self.personas['John Lin'].perceive_suggestion_eval(self.personas, action, first_suggestion, eval_res_score, eval_res_reason, deducted_reason, self.train_mode)
                      self.personas['Eddy Lin'].scratch.suggestion_eval_result = eval_res_score
                      
                    if eval_res_score_2:
                      #### only by score prefernece
                      if float(eval_res_score) > float(eval_res_score_2):
                        better_num = '[#1]'
                      elif float(eval_res_score) < float(eval_res_score_2):
                        better_num = '[#2]'
                      else:
                        better_num = '[]'
                      persona.update_preference_data(self.train_mode, persona.current_prompt, (eval_res_score, eval_res_reason), (eval_res_score_2, eval_res_reason_2), better_num, "", first_suggestion, suggestion)
                      date = self.curr_time.strftime("%B %d, %Y")
                      persona.update_total_records(date, action, (first_suggestion, suggestion), eval_res_reason, eval_res_reason_2, deducted_reason)

                else:
                  rand_num = random.random()
                  if 'norec100' in self.train_mode:
                    first_suggestion = "No Recommendation"
                  elif 'norec70' in self.train_mode and rand_num > 0.3:
                    first_suggestion = "No Recommendation"
                  elif 'norec30' in self.train_mode and rand_num > 0.7:
                    first_suggestion = "No Recommendation"
                  else:
                    action, first_suggestion = persona.suggest(self.personas, self.train_mode, is_john_walking=is_john_walking, movement_path=f"{sim_folder}/movement/", 
                                                    step=self.step, curr_time=self.curr_time.strftime("%A %B %d -- %I:%M:%S %p"))
                  ...
                  if suggestion:
                    eval_score_reason_1, eval_score_reason_2, deducted_reason = self.personas['John Lin'].eval_suggestion(self.personas, train_mode=self.train_mode, first_suggestion=first_suggestion)
                    better_num = ''
                    if eval_score_reason_2:
                      #### only by score prefernece
                      if float(eval_score_reason_1[0]) > float(eval_score_reason_2[0]):
                        better_num = '[#1]'
                      elif float(eval_score_reason_1[0]) < float(eval_score_reason_2[0]):
                        better_num = '[#2]'
                      else:
                        better_num = '[]'
                      persona.update_preference_data(self.train_mode, persona.current_prompt, eval_score_reason_1, eval_score_reason_2, better_num, "", first_suggestion, suggestion)
                      date = self.curr_time.strftime("%B %d, %Y")
                      persona.update_total_records(date, action, (first_suggestion, suggestion), eval_score_reason_1, eval_score_reason_2, deducted_reason)
                      eval_res_score = eval_score_reason_2[0]
                      eval_res_reason = eval_score_reason_2[1]
              else:
                action, suggestion = persona.suggest(self.personas, self.train_mode, is_john_walking=is_john_walking, movement_path=f"{sim_folder}/movement/", 
                                                    step=self.step, curr_time=self.curr_time.strftime("%A %B %d -- %I:%M:%S %p"))
                              
                eval_res_score, eval_res_reason, sugg_revision = self.personas['John Lin'].eval_suggestion(self.personas, train_mode=self.train_mode)
                if eval_res_score:
                  persona.update_preference_data(self.train_mode, persona.current_prompt, suggestion, eval_res_reason, eval_res_score, sugg_revision)
                  date = self.curr_time.strftime("%B %d, %Y")
                  persona.update_total_records(date, action, suggestion, eval_res_score, eval_res_reason, sugg_revision)

              persona.scratch.suggestion = None
              # persona.scratch.user_action_for_suggestion = None
              persona.scratch.suggestion_eval_result = None

              next_tile, pronunciatio, description = persona.move(
                self.maze, self.personas, self.personas_tile[persona_name], 
                self.curr_time, None, None)
            else:
              next_tile, pronunciatio, description = persona.move(
                self.maze, self.personas, self.personas_tile[persona_name], 
                self.curr_time)
              if next_tile != john_tile:
                is_john_walking = 'yes'
              else:
                is_john_walking = 'no'
              john_tile = next_tile

            if 'recrev' in self.train_mode and 'ranking' in self.train_mode and first_suggestion != "":
            ...
            movements["persona"][persona_name]["chat"] = (persona
                                                          .scratch.chat)
            movements["persona"][persona_name]["user_action_for_suggestion"] = action
            movements["persona"][persona_name]["suggestion"] = suggestion
            movements["persona"][persona_name]["eval_res_score"] = eval_res_score
            movements["persona"][persona_name]["eval_res_reason"] = eval_res_reason
          # Include the meta information about the current stage in the 
          # movements dictionary. 
          movements["meta"]["curr_time"] = (self.curr_time 
                                             .strftime("%B %d, %Y, %H:%M:%S"))
          movements["meta"]["is_john_walking"] = is_john_walking
          if not suggestion:
            suggestion = ""
          
          if not eval_res_reason:
            eval_res_reason = ""
            
          if is_john_walking == 'yes':
            movements["meta"]["number_of_action_tokens"] = len(logging_tokenizer(suggestion).input_ids) + len(logging_tokenizer(eval_res_reason).input_ids) + 4
          else:
            if action != None:
              movements["meta"]["number_of_action_tokens"] = len(logging_tokenizer(suggestion).input_ids) + len(logging_tokenizer(eval_res_reason).input_ids) + len(logging_tokenizer('John Lin is '+action).input_ids)-1
            else:
              movements["meta"]["number_of_action_tokens"] = 0
  ...
  def load_logging_tokenizer(self):
    tokenizer = AutoTokenizer.from_pretrained("meta-llama/Llama-3.3-70B-Instruct")
    return tokenizer
  
  def generate_fake_environment(self):
    """
    Generates environment files that would normally come from the frontend.
    This allows the backend to run independently without frontend interaction.
    Also updates agent positions based on their planned movements.
    
    INPUT
      None
    OUTPUT
      None
      * Creates environment JSON files with updated persona positions
    """
    ...
    if prev_step >= 0:
        movement_file = f"{fs_storage}/{self.sim_code}/movement/{prev_step}.json"
        if os.path.exists(movement_file):
            try:
                with open(movement_file) as f:
                    movements = json.load(f)
                
                for persona_name, data in movements["persona"].items():
                    if "movement" in data and data["movement"]:
                        if isinstance(data["movement"], list):
                            self.personas_tile[persona_name] = (data["movement"][0], data["movement"][1])
                        else:
                            self.personas_tile[persona_name] = data["movement"]
            except Exception as e:
                print(f"Error applying movements: {e}")
    
    env = {}
    for name, tile in self.personas_tile.items():
        env[name] = {"maze": self.maze.maze_name, "x": tile[0], "y": tile[1]}
    
    env_file = f"{fs_storage}/{self.sim_code}/environment/{self.step}.json"
    ...
    while True:
      ...
      try: 
        if sim_command.lower() in ["f", "fin", "finish", "save and finish"]: 
          ...
          load_history_via_whisper(self.personas, clean_whispers, client=self.client)
...
if __name__ == '__main__':
  ...
  origin = ORIGIN_PERSONA
  # origin = "base_the_ville_specific_john_agent"
  # origin = "base_the_ville_specific_carlos_agent"
  target = SAVE_DIR
