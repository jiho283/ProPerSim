...
from trl import KTOConfig, KTOTrainer, extract_prompt, unpair_preference_dataset
...
def get_unpaired_dataset(
    data_dir: str = "./sim_training_data.pkl",
) -> Dataset:
    ...
    def filter_abstention(example):
        return 'Abstention' not in example['completion'][0]['content']
...
if __name__ == "__main__":
    ...
    train_dataset = get_unpaired_dataset(data_dir=script_args.train_data_dir)
    # import pdb;pdb.set_trace()
    train_dataset = train_dataset.filter(
        lambda x: len(x["prompt"]) + len(x["completion"]) <= script_args.max_length,
        num_proc=2,
    )
    ...
    training_args = KTOConfig(
        ...
        run_name="kto_llama3.3-no-ds-new_data_no_reason",
